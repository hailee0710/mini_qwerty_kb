...
Letters layer, 3 rows. Row 1 is 10 columns — `X(Z) F(P) C S(V) W(Y) A I U(K) J ,(.)` — comma on top, dot below it. Row 2 is 9 columns: `R(Q) D T(B) H(M) N(L) O E G ⌫`. All five Telex tone keys (X, F, S, J, R) sit on primary (top) slots; Z, P, V, Y, K, Q, B, M, L are the double-tap secondaries. The layout is corpus-optimized by `tools/layout_analyzer.py` (word-frequency data in `tools/data/vi_50k.txt`); A, E, O, D sit on keys without secondaries so the Telex same-key digraphs `aa ee oo dd` stay typeable via quick double-press. Row 3 uses fractional `widthUnits` spans (shift 1.5, `123` 1, space 5, return 1.5). `123` switches to the numeric layer; `ABC` switches back.
